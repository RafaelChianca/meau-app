...
export default function OptionNotification({ notification, headerColor = '#cfe9e5', ...rest }) {
  ...
  const accept = () => {
    Alert.alert(
      ...
      [
          { text: 'Cancelar' },
          { text: "Ok", onPress: () => dispatch(acceptAdoptionRequested(notification.targetID, notification.adopter, notification.petID)) }
      ],
      { cancelable: true }
    )
  }
  ...
  const decline = () => {
    Alert.alert(
      ...
      [
          { text: 'Cancelar' },
          { text: "Ok", onPress: () => dispatch(declineAdoptionRequested(notification.targetID, notification.adopter, notification.petID)) }
      ],
      { cancelable: true }
    )
  }
