...
export default function OptionNotification({ notification, headerColor = '#cfe9e5', ...rest }) {
  ...
  const accept = () => {
    Alert.alert(
      ...
      [
          { text: 'Cancelar' },
          { text: "Ok", onPress: () => dispatch(acceptAdoptionRequested(notification.id, notification.petID, notification.adopterID)) }
      ],
      { cancelable: true }
    )
  }
  ...
  const decline = () => {
    Alert.alert(
      ...
      [
          { text: 'Cancelar' },
          { text: "Ok", onPress: () => dispatch(declineAdoptionRequested(notification.id, notification.adopterID)) }
      ],
      { cancelable: true }
    )
  }
