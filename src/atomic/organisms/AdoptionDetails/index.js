...
import { Container, PetImage, FavButton, TextGroup, Titulo, Group, Linha } from './styles';
import EntIcon from 'react-native-vector-icons/Entypo';
import EvilIcon from 'react-native-vector-icons/EvilIcons';
import CustomHeader from '../../molecules/CustomHeader';
import CustomButton from '../../atoms/CustomButton';

export default function AdoptDetails({route, ... rest}) {
    ...
    return(
        <Container>
            ...
            <PetImage source={{uri: route.params.pet.species === 'Cachorro' ? dogPicture : catPicture}}></PetImage>
            <FavButton>
                <EvilIcon name="heart" color="#434343" size={48}/>
            </FavButton>
            <Text style={{fontSize:16, color:"#434343", fontWeight: 'bold', marginLeft: 30, marginTop: -24, marginBottom: 16 }}>{route.params.pet.name}</Text>
            <Group>
                <TextGroup>
                    <Titulo>Sexo</Titulo>
                    <Text>{route.params.pet.sex}</Text>
                </TextGroup>
                <TextGroup>
                    <Titulo>Porte</Titulo>
                    <Text>{route.params.pet.size}</Text>
                </TextGroup>
                <TextGroup>
                    <Titulo>Idade</Titulo>
                    <Text>{route.params.pet.age}</Text>
                </TextGroup>
            </Group>
            <Group>
                <TextGroup>
            ...
            <Group>
                <TextGroup>
                    ...
                    <Text>{route.params.pet.health?.includes('Castrado')?'Sim':'Não'}</Text>
                </TextGroup>
                <TextGroup>
                    <Titulo>vermifugado</Titulo>
                    <Text>{route.params.pet.health?.includes('Vermifugado')?'Sim':'Não'}</Text>
                </TextGroup>
            </Group>
            <Group>
                <TextGroup>
                    <Titulo>vacinado</Titulo>
                    <Text>{route.params.pet.health?.includes('Vacinado')?'Sim':'Não'}</Text>
                </TextGroup>
                <TextGroup>
                    <Titulo>doenças</Titulo>
                    <Text>{route.params.pet.diseases}</Text>
                </TextGroup>
            </Group>
            <Linha/>
            <Group>
                <TextGroup>
                    ...
                    <Text>{route.params.pet.temperment.join(', ')}</Text>
                </TextGroup>
            </Group>
            <Linha/>
            <Group>
                <TextGroup>
                    ...
                    <Text>{route.params.pet.conditions.join(', ')}</Text>
                </TextGroup>
            </Group>
            <Linha/>
            <Group>
                <TextGroup>
                    <Titulo>mais sobre {route.params.pet.name}</Titulo>
                    <Text>{route.params.pet.about}</Text>
                </TextGroup>
            </Group>
